...
Elm.Erl = Elm.Erl || {};
Elm.Erl.make = function (_elm) {
   "use strict";
   _elm.Erl = _elm.Erl || {};
   if (_elm.Erl.values) return _elm.Erl.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   ...
   var addQuery = F3(function (key,val,url) {
      var updated = $String.isEmpty(val) ? A2($Dict.remove,key,url.query) : A3($Dict.insert,key,val,url.query);
   ...
   var queryTuples = function (queryString) {
      var splitted = A2($String.split,"&",queryString);
      return $String.isEmpty(queryString) ? _U.list([]) : A2($List.map,queryStringElementToTuple,splitted);
   };
   ...
   var extractPort = function (str) {
      ...
      80,
      $Result.toMaybe($String.toInt(A2($String.dropLeft,1,A2($Maybe.withDefault,"",$List.head(A2($List.map,function (_) {    return _.match;},res)))))));
   };
   ...
   var leftFromOrSame = F2(function (delimiter,str) {    var parts = A2($String.split,delimiter,str);return A2($Maybe.withDefault,"",$List.head(parts));});
   var rightFromOrSame = F2(function (delimiter,str) {
      var parts = A2($String.split,delimiter,str);
      return A2($Maybe.withDefault,"",$List.head($List.reverse(parts)));
   });
   var extractHost = function (str) {
      var localhostRx = "localhost";
      var dotsRx = "((\\w|-)+\\.)+(\\w|-)+";
      var rx = A2($Basics._op["++"],"(",A2($Basics._op["++"],dotsRx,A2($Basics._op["++"],"|",A2($Basics._op["++"],localhostRx,")"))));
      return A2($Maybe.withDefault,
      "",
      ...
      A3($Regex.find,$Regex.AtMost(1),$Regex.regex(rx),A2(leftFromOrSame,"/",A2(rightFromOrSame,"//",str))))));
   };
   var host = function (str) {    return parseHost(extractHost(str));};
   var extractPath = function (str) {
      var host = extractHost(str);
   ...
   var Url = F8(function (a,b,c,d,e,f,g,h) {    return {protocol: a,username: b,password: c,host: d,port$: e,path: f,hash: g,query: h};});
   return _elm.Erl.values = {_op: _op
                            ,addQuery: addQuery
...
Elm.OnePageStack.Types = Elm.OnePageStack.Types || {};
Elm.OnePageStack.Types.make = function (_elm) {
   "use strict";
   _elm.OnePageStack = _elm.OnePageStack || {};
   _elm.OnePageStack.Types = _elm.OnePageStack.Types || {};
   if (_elm.OnePageStack.Types.values) return _elm.OnePageStack.Types.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   ...
   return _elm.OnePageStack.Types.values = {_op: _op,Page: Page,PageLoading: PageLoading,PageNotFound: PageNotFound,AppInterface: AppInterface};
};
Elm.Html = Elm.Html || {};
...
Elm.OnePageStack = Elm.OnePageStack || {};
Elm.OnePageStack.Server = Elm.OnePageStack.Server || {};
Elm.OnePageStack.Server.make = function (_elm) {
   "use strict";
   _elm.OnePageStack = _elm.OnePageStack || {};
   _elm.OnePageStack.Server = _elm.OnePageStack.Server || {};
   if (_elm.OnePageStack.Server.values) return _elm.OnePageStack.Server.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   ...
   var currentLocation = A2($Signal.map2,
   F2(function ($new,initial) {
      var url = $Erl.parse(initial);
      var _p0 = $new;
      if (_p0.ctor === "Nothing") {
            return url;
         } else {
            return _U.update(url,{query: _p0._0});
         }
   }),
   locationChanger.signal);
   var contentHook = $Signal.mailbox($OnePageStack$Types.PageLoading);
   var interfaceSignal = $Signal.map(A2($OnePageStack$Types.AppInterface,contentHook.address,locationChanger.address));
   var view = function (p) {
      var _p1 = p;
      switch (_p1.ctor)
   ...
   var handleRequest = F3(function (defaultProvider,providers,$interface) {
      var provider = A2($Maybe.withDefault,
      defaultProvider,
      A2($Maybe.andThen,A2($Dict.get,"type",$interface.currentUrl.query),A2($Basics.flip,$Dict.get,providers)));
      return A2($Task.onError,
      A2($Task.andThen,provider($interface),function (_p2) {    return A2($Signal.send,$interface.canvas,$OnePageStack$Types.Page(_p2));}),
      function (_p3) {
         return A2($Signal.send,$interface.canvas,$OnePageStack$Types.PageNotFound($Basics.toString(_p3)));
      });
   });
   var server = F2(function (defaultProvider,p) {
      return function (_p4) {
         return A2($Signal.map,A2(handleRequest,defaultProvider,p),interfaceSignal(_p4));
      };
   });
   return _elm.OnePageStack.Server.values = {_op: _op
...
Elm.OnePageStack.Provider.Util = Elm.OnePageStack.Provider.Util || {};
Elm.OnePageStack.Provider.Util.make = function (_elm) {
   "use strict";
   _elm.OnePageStack = _elm.OnePageStack || {};
   _elm.OnePageStack.Provider = _elm.OnePageStack.Provider || {};
   _elm.OnePageStack.Provider.Util = _elm.OnePageStack.Provider.Util || {};
   if (_elm.OnePageStack.Provider.Util.values) return _elm.OnePageStack.Provider.Util.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var withTemplate = F3(function (template,f,$interface) {    return A2($Task.map,template,f($interface));});
   var navigate = F2(function (s1,s2) {
      return $Maybe.Just($Dict.fromList(_U.list([{ctor: "_Tuple2",_0: "type",_1: s1},{ctor: "_Tuple2",_0: "page",_1: s2}])));
   });
   var mkProvider = F3(function (fetchTask,renderer,$interface) {
      var url = $interface.currentUrl;
      return A2($Task.andThen,
      $History.setPath($Erl.toString(url)),
      function (_p0) {
         return A2($Task.andThen,fetchTask(url.query),function (_p1) {    return $Task.succeed(A2(renderer,$interface.navigator,_p1));});
      });
   });
   return _elm.OnePageStack.Provider.Util.values = {_op: _op,mkProvider: mkProvider,navigate: navigate,withTemplate: withTemplate};
};
Elm.Path = Elm.Path || {};
Elm.Path.Generic = Elm.Path.Generic || {};
Elm.Path.Generic.make = function (_elm) {
   "use strict";
   _elm.Path = _elm.Path || {};
   _elm.Path.Generic = _elm.Path.Generic || {};
   if (_elm.Path.Generic.values) return _elm.Path.Generic.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   var _op = {};
   var extSeparator = ".";
   var isExtSeparator = F2(function (x,y) {    return _U.eq(x,y);})(extSeparator);
   var splitExtension = function (path) {
      var _p0 = A2($String.split,extSeparator,$String.reverse(path));
      if (_p0.ctor === "[]") {
            return {ctor: "_Tuple2",_0: "",_1: ""};
         } else {
            if (_p0._1.ctor === "[]") {
                  return {ctor: "_Tuple2",_0: $String.reverse(_p0._0),_1: ""};
               } else {
                  return {ctor: "_Tuple2"
                         ,_0: $String.reverse(A2($String.join,extSeparator,_p0._1))
                         ,_1: A2($Basics._op["++"],extSeparator,$String.reverse(_p0._0))};
               }
         }
   };
   var takeExtension = function (_p1) {    return $Basics.snd(splitExtension(_p1));};
   var dropExtension = function (_p2) {    return $Basics.fst(splitExtension(_p2));};
   var hasExtension = function (_p3) {    return $String.isEmpty($Basics.snd(splitExtension(_p3)));};
   var splitExtensions = function (path) {
      var _p4 = A2($String.split,extSeparator,path);
      if (_p4.ctor === "[]") {
            return {ctor: "_Tuple2",_0: "",_1: ""};
         } else {
            if (_p4._1.ctor === "[]") {
                  ...
                  return {ctor: "_Tuple2",_0: _p4._0,_1: A2($String.join,extSeparator,_p4._1)};
               }
         }
   };
   var takeExtensions = function (_p5) {    return $Basics.snd(splitExtensions(_p5));};
   var dropExtensions = function (_p6) {    return $Basics.fst(splitExtensions(_p6));};
   var normalizeExt = function (ext) {    return A2($String.startsWith,extSeparator,ext) ? ext : A2($Basics._op["++"],extSeparator,ext);};
   var replaceExtension = F2(function (path,ext) {
      return A2($Basics.flip,F2(function (x,y) {    return A2($Basics._op["++"],x,y);}),normalizeExt(ext))($Basics.fst(splitExtension(path)));
   });
   _op["-<.>"] = replaceExtension;
   var addExtension = function (path) {
      return function (_p7) {
         return A2(F2(function (x,y) {    return A2($Basics._op["++"],x,y);}),path,normalizeExt(_p7));
      };
   };
   _op["<.>"] = addExtension;
   var pathSeparators = function (platform) {
      var _p8 = platform;
      if (_p8.ctor === "Windows") {
            return _U.list(["/","\\"]);
         } else {
   ...
   var isPathSeparator = function (platform) {    return A2($Basics.flip,$List.member,pathSeparators(platform));};
   var hasTrailingPathSeparator = F2(function (platform,path) {
      return A2($List.any,A2($Basics.flip,$String.endsWith,path),pathSeparators(platform)) && $Basics.not(A2(isPathSeparator,platform,path));
   });
   var pathSeparator = function (platform) {    var _p9 = platform;if (_p9.ctor === "Windows") {    return "\\";} else {    return "/";}};
   var splitFileName = F2(function (platform,path) {
      var _p10 = A2($String.split,pathSeparator(platform),$String.reverse(path));
      if (_p10.ctor === "[]") {
            return {ctor: "_Tuple2",_0: "",_1: ""};
         } else {
            if (_p10._1.ctor === "[]") {
                  ...
                  return {ctor: "_Tuple2"
                         ,_0: $String.reverse(A2($String.join,pathSeparator(platform),A2($List._op["::"],"",_p10._1)))
                         ,_1: $String.reverse(_p10._0)};
               }
         }
   });
   var takeFileName = function (platform) {    return function (_p11) {    return $Basics.snd(A2(splitFileName,platform,_p11));};};
   var takeBaseName = function (platform) {    return function (_p12) {    return dropExtension(A2(takeFileName,platform,_p12));};};
   var replaceDirectory = F2(function (platform,path) {
      return A2($Basics.flip,F2(function (x,y) {    return A2($Basics._op["++"],x,y);}),A2(takeFileName,platform,path));
   });
   var dropFileName = function (platform) {    return function (_p13) {    return $Basics.fst(A2(splitFileName,platform,_p13));};};
   var replaceFileName = F2(function (platform,path) {    return F2(function (x,y) {    return A2($Basics._op["++"],x,y);})(A2(dropFileName,platform,path));});
   var replaceBaseName = F3(function (platform,path,$new) {
      var _p14 = A2(splitFileName,platform,path);
      var dir = _p14._0;
      var old = _p14._1;
      var _p15 = splitExtension(old);
      var ext = _p15._1;
      return A2($Basics._op["++"],dir,A2($Basics._op["++"],$new,ext));
   });
   var takeDirectory = function (platform) {    return function (_p16) {    return $Basics.fst(A2(splitFileName,platform,_p16));};};
   var splitPath = F2(function (platform,path) {
      var _p17 = A2($String.split,pathSeparator(platform),path);
      if (_p17.ctor === "::") {
            var _p19 = _p17._1;
            var _p18 = _p17._0;
            return $String.isEmpty(_p18) ? A2($List._op["::"],pathSeparator(platform),_p19) : A2($List._op["::"],_p18,_p19);
         } else {
            return _p17;
         }
   });
   var addTrailingPathSeparator = F2(function (platform,path) {
      return A2(hasTrailingPathSeparator,platform,path) ? path : A2($Basics._op["++"],path,pathSeparator(platform));
   });
   var combine = F3(function (platform,path1,path2) {
      if (A2($List.any,A2($Basics.flip,$String.startsWith,path2),pathSeparators(platform))) return path2; else {
            ...
            if (_p20._0 === "") {
                  ...
                  if (_p20._1 === "") {
                        ...
                        var _p22 = _p20._1;
                        var _p21 = _p20._0;
                        return A2(isPathSeparator,platform,_p21) ? A2($Basics._op["++"],_p21,_p22) : A2($Basics._op["++"],
                        A2(addTrailingPathSeparator,platform,_p21),
                        _p22);
                     }
               }
         }
   });
   var joinPath = function (_p23) {    return A3($Basics.flip,$List.foldr,"",combine(_p23));};
   var dropTrailingPathSeparator = F2(function (platform,path) {
      return A2(hasTrailingPathSeparator,platform,path) ? A2($Maybe.withDefault,
      path,
      A2($Maybe.map,function (_p24) {    return A2(joinPath,platform,$List.reverse(_p24));},$List.tail($List.reverse(A2(splitPath,platform,path))))) : path;
   });
   var Url = {ctor: "Url"};
   var Windows = {ctor: "Windows"};
   var Posix = {ctor: "Posix"};
   return _elm.Path.Generic.values = {_op: _op
...
Elm.Path = Elm.Path || {};
Elm.Path.Url = Elm.Path.Url || {};
Elm.Path.Url.make = function (_elm) {
   "use strict";
   _elm.Path = _elm.Path || {};
   _elm.Path.Url = _elm.Path.Url || {};
   if (_elm.Path.Url.values) return _elm.Path.Url.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Path$Generic = Elm.Path.Generic.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var dropExtensions = $Path$Generic.dropExtensions;
   var takeExtensions = $Path$Generic.takeExtensions;
   var splitExtensions = $Path$Generic.splitExtensions;
   var hasExtension = $Path$Generic.hasExtension;
   var addExtension = $Path$Generic.addExtension;
   _op["<.>"] = addExtension;
   var dropExtension = $Path$Generic.dropExtension;
   var replaceExtension = $Path$Generic.replaceExtension;
   _op["-<.>"] = replaceExtension;
   var takeExtension = $Path$Generic.takeExtension;
   var splitExtension = $Path$Generic.splitExtension;
   var isExtSeparator = $Path$Generic.isExtSeparator;
   var extSeparator = $Path$Generic.extSeparator;
   var currPlatform = $Path$Generic.Url;
   var pathSeparator = $Path$Generic.pathSeparator(currPlatform);
   var pathSeparators = $Path$Generic.pathSeparators(currPlatform);
   var isPathSeparator = $Path$Generic.isPathSeparator(currPlatform);
   var splitFileName = $Path$Generic.splitFileName(currPlatform);
   var takeFileName = $Path$Generic.takeFileName(currPlatform);
   var replaceFileName = $Path$Generic.replaceFileName(currPlatform);
   var dropFileName = $Path$Generic.dropFileName(currPlatform);
   var takeBaseName = $Path$Generic.takeBaseName(currPlatform);
   var replaceBaseName = $Path$Generic.replaceBaseName(currPlatform);
   var takeDirectory = $Path$Generic.takeDirectory(currPlatform);
   var replaceDirectory = $Path$Generic.replaceDirectory(currPlatform);
   var combine = $Path$Generic.combine(currPlatform);
   _op["</>"] = combine;
   var splitPath = $Path$Generic.splitPath(currPlatform);
   var joinPath = $Path$Generic.joinPath(currPlatform);
   var hasTrailingPathSeparator = $Path$Generic.hasTrailingPathSeparator(currPlatform);
   var addTrailingPathSeparator = $Path$Generic.addTrailingPathSeparator(currPlatform);
   var dropTrailingPathSeparator = $Path$Generic.dropTrailingPathSeparator(currPlatform);
   return _elm.Path.Url.values = {_op: _op
                                 ,pathSeparator: pathSeparator
                                 ,pathSeparators: pathSeparators
                                 ,isPathSeparator: isPathSeparator
                                 ,extSeparator: extSeparator
...
Elm.OnePageStack = Elm.OnePageStack || {};
Elm.OnePageStack.Provider = Elm.OnePageStack.Provider || {};
Elm.OnePageStack.Provider.Post = Elm.OnePageStack.Provider.Post || {};
Elm.OnePageStack.Provider.Post.make = function (_elm) {
   "use strict";
   _elm.OnePageStack = _elm.OnePageStack || {};
   _elm.OnePageStack.Provider = _elm.OnePageStack.Provider || {};
   _elm.OnePageStack.Provider.Post = _elm.OnePageStack.Provider.Post || {};
   if (_elm.OnePageStack.Provider.Post.values) return _elm.OnePageStack.Provider.Post.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var renderPost = function (_p0) {    return $Markdown.toHtml;};
   var fetchPost = F2(function (basePath,params) {
      var _p1 = A2($Dict.get,"page",params);
      if (_p1.ctor === "Nothing") {
            return $Task.fail("No page specified");
         } else {
            return A2($Task.mapError,$Basics.toString,$Http.getString(A2($Path$Url._op["</>"],basePath,_p1._0)));
         }
   });
   var postProvider = function (basePath) {    return A2($OnePageStack$Provider$Util.mkProvider,fetchPost(basePath),renderPost);};
   return _elm.OnePageStack.Provider.Post.values = {_op: _op,postProvider: postProvider};
};
Elm.Native.Date = {};
...
Elm.OnePageStack.Provider.Index.make = function (_elm) {
   "use strict";
   _elm.OnePageStack = _elm.OnePageStack || {};
   _elm.OnePageStack.Provider = _elm.OnePageStack.Provider || {};
   _elm.OnePageStack.Provider.Index = _elm.OnePageStack.Provider.Index || {};
   if (_elm.OnePageStack.Provider.Index.values) return _elm.OnePageStack.Provider.Index.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var renderIndex = function (chgr) {
      return function (_p0) {
         return A2($Html.ul,
         _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "padding-left",_1: "10px"}]))]),
         A2($List.map,
         function (pm) {
            return A2($Html.li,
            _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "list-style-type",_1: "none"},{ctor: "_Tuple2",_0: "padding",_1: "0"}]))]),
            _U.list([A2($Html.a,
            _U.list([A2($Html$Events.onClick,chgr,A2($OnePageStack$Provider$Util.navigate,"post",pm.location))]),
            A2($Basics._op["++"],
            _U.list([A2($Html.h3,_U.list([]),_U.list([$Html.text(pm.title)]))]),
            function () {
               var _p1 = pm.description;
               if (_p1.ctor === "Nothing") {
                     return _U.list([]);
                  } else {
                     return _U.list([A2($Html.p,_U.list([]),_U.list([$Html.text(_p1._0)]))]);
                  }
            }()))]));
         },
         $List.reverse(A2($List.sortBy,function (_p2) {    return $Date.toTime(function (_) {    return _.date;}(_p2));},_p0))));
      };
   };
   var PostMeta = F4(function (a,b,c,d) {    return {location: a,title: b,description: c,date: d};});
   var postMetaDecoder = A5($Json$Decode.object4,
   PostMeta,
   A2($Json$Decode._op[":="],"location",$Json$Decode.string),
   ...
   function (s) {
      var _p3 = $Date.fromString(s);
      if (_p3.ctor === "Err") {
            return $Json$Decode.fail(_p3._0);
         } else {
            return $Json$Decode.succeed(_p3._0);
         }
   }));
   var fetchIndex = F2(function (basePath,_p4) {
      return A2($Task.mapError,$Basics.toString,A2($Http.get,$Json$Decode.list(postMetaDecoder),A2($Path$Url._op["</>"],basePath,"posts.json")));
   });
   var indexProvider = function (basePath) {    return A2($OnePageStack$Provider$Util.mkProvider,fetchIndex(basePath),renderIndex);};
   return _elm.OnePageStack.Provider.Index.values = {_op: _op,indexProvider: indexProvider};
};
Elm.OnePageStack = Elm.OnePageStack || {};
Elm.OnePageStack.Template = Elm.OnePageStack.Template || {};
Elm.OnePageStack.Template.make = function (_elm) {
   "use strict";
   _elm.OnePageStack = _elm.OnePageStack || {};
   _elm.OnePageStack.Template = _elm.OnePageStack.Template || {};
   if (_elm.OnePageStack.Template.values) return _elm.OnePageStack.Template.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var withTemplate = F3(function (t,f,i) {    return A2($Task.andThen,f(i),t(i));});
   var acquired = F2(function (html,_p0) {    return A2($Task.mapError,$Basics.toString,$Task.succeed($Maybe.Just(html)));});
   var $return = F3(function (a,_p2,_p1) {    return $Task.succeed(a);});
   var render = F3(function (a,_p3,h) {    return $Task.succeed(a(h));});
   var andThen = F4(function (ta,f,i,html) {    return A2($Task.andThen,A2(ta,i,html),function (a) {    return A3(f,a,i,html);});});
   var map = F4(function (f,tb,i,h) {    return A2($Task.map,f,A2(tb,i,h));});
   var getInterface = F2(function (i,_p4) {    return $Task.succeed(i);});
   var getUrl = A2(map,function (_) {    return _.currentUrl;},getInterface);
   var acquire = function (component) {    return A2(andThen,getUrl,F3(function (url,_p6,_p5) {    return component(url);}));};
   var getNavigator = A2(map,function (_) {    return _.navigator;},getInterface);
   var getCanvas = A2(map,function (_) {    return _.canvas;},getInterface);
   var nest = F4(function (outer,inner,$interface,content) {    return A2($Task.andThen,A2(inner,$interface,content),outer($interface));});
   return _elm.OnePageStack.Template.values = {_op: _op
                                              ,nest: nest
...
Elm.Util.make = function (_elm) {
   ...
   var $const = F2(function (a,_p0) {    return a;});
   var unless = function (b) {    return b ? $const($Task.succeed({ctor: "_Tuple0"})) : $Basics.identity;};
   var when = function (_p1) {    return unless($Basics.not(_p1));};
   return _elm.Util.values = {_op: _op,$const: $const,unless: unless,when: when,singleton: singleton};
};
Elm.Template = Elm.Template || {};
Elm.Template.make = function (_elm) {
   "use strict";
   _elm.Template = _elm.Template || {};
   if (_elm.Template.values) return _elm.Template.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Erl = Elm.Erl.make(_elm),
   ...
   $Markdown = Elm.Markdown.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $OnePageStack$Template = Elm.OnePageStack.Template.make(_elm),
   $Path$Url = Elm.Path.Url.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Task = Elm.Task.make(_elm),
   ...
   var headerStyle = _U.list([A2(_op[":="],"top","0")
                             ,A2(_op[":="],"width","100%")
                             ,A2(_op[":="],"background-color","rgb(62, 62, 62)")
                             ,A2(_op[":="],"color","white")]);
   var headerImpl = A2($OnePageStack$Template.andThen,
   $OnePageStack$Template.getInterface,
   function (_p0) {
      var _p1 = _p0;
      var query_ = function (_) {    return _.query;}(A2($Erl.removeQuery,"type",A2($Erl.removeQuery,"page",_p1.currentUrl)));
      return $OnePageStack$Template.$return(A2($Html.header,
      _U.list([$Html$Attributes.style(headerStyle)]),
      _U.list([A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([A2(_op[":="],"padding","10px 15px")]))]),
      _U.list([A2($Html.a,_U.list([A2($Html$Events.onClick,_p1.navigator,$Maybe.Just(query_))]),_U.list([$Html.text("Justus\'s homepage v3.0")]))]))])));
   });
   var footerBlock = function (inner) {
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([A2(_op[":="],"width","33%"),A2(_op[":="],"float","left")]))]),
      _U.list([A2($Html.div,_U.list([$Html$Attributes.style(_U.list([A2(_op[":="],"margin","15px 15px")]))]),inner)]));
   };
   var centerBlock = _U.list([A2(_op[":="],"width","960px"),A2(_op[":="],"margin-left","auto"),A2(_op[":="],"margin-right","auto")]);
   var pageTemplate = A2($OnePageStack$Template.andThen,
   headerImpl,
   function (header) {
      return $OnePageStack$Template.render(function (main) {
         return A2($Html.div,
         _U.list([]),
         _U.list([header
                 ,A2($Html.div,_U.list([$Html$Attributes.style(centerBlock)]),_U.list([main]))
                 ,A2($Html.footer,
                 _U.list([$Html$Attributes.style(_U.list([A2(_op[":="],"margin","20px"),A2(_op[":="],"border","1px solid rgb(233, 233, 233)")]))]),
                 _U.list([A2($Html.div,
                 _U.list([$Html$Attributes.style(centerBlock)]),
                 _U.list([footerBlock(_U.list([$Html.text("This site is built with the wonderful ")
                                              ,A2($Html.a,_U.list([$Html$Attributes.href("http://elm-lang.org")]),_U.list([$Html.text("Elm")]))
                                              ,$Html.text(" language")]))
                         ,footerBlock(_U.list([$Html.text("")]))
                         ,footerBlock(_U.list([$Html.text("©️ 2016 Justus Adam")]))]))]))]));
      });
   });
   var postTemplate = A2($OnePageStack$Template.nest,
   pageTemplate,
   $OnePageStack$Template.render(function (main) {    return A2($Html.div,_U.list([]),_U.list([A2($Html.section,_U.list([]),_U.list([main]))]));}));
   var sidebar = F2(function (basePath,_p2) {
      return A2($Task.mapError,
      $Basics.toString,
      A2($Task.map,
      function (_p3) {
         return $Maybe.Just(A2($Html.ul,
         _U.list([]),
         A2($List.map,
         function (_p4) {
            return A2($Html.li,
            _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "list-style-type",_1: "none"},{ctor: "_Tuple2",_0: "margin",_1: "none"}]))]),
            $Util.singleton($Markdown.toHtml(_p4)));
         },
         _p3)));
      },
      A2($Http.get,$Json$Decode.list($Json$Decode.string),A2($Path$Url._op["</>"],basePath,"sidebar.json"))));
   });
   var indexTemplate = function (basePath) {
      return A2($OnePageStack$Template.andThen,
      $OnePageStack$Template.acquire(sidebar(basePath)),
      function (sb) {
         return A2($OnePageStack$Template.nest,
         pageTemplate,
         $OnePageStack$Template.render(function (main) {
            return A2($Html.div,
            _U.list([]),
            _U.list([A2($Html.section,_U.list([$Html$Attributes.style(_U.list([A2(_op[":="],"float","left"),A2(_op[":="],"width","66%")]))]),_U.list([main]))
                    ,A2($Html.div,
                    _U.list([$Html$Attributes.style(_U.list([A2(_op[":="],"float","left"),A2(_op[":="],"width","33%")]))]),
                    _U.list([A2($Html.div,
                    _U.list([$Html$Attributes.style(_U.list([A2(_op[":="],"margin","20px")]))]),
                    function (a) {
                       return _U.list([a]);
                    }(A2($Maybe.withDefault,$Html.text("no sidebar"),sb)))]))
                    ,A2($Html.div,_U.list([$Html$Attributes.style(_U.list([A2(_op[":="],"clear","both")]))]),_U.list([]))]));
         }));
      });
   };
   return _elm.Template.values = {_op: _op
                                 ,sidebar: sidebar
...
Elm.Main.make = function (_elm) {
   ...
   var _op = {};
   var locationIn = Elm.Native.Port.make(_elm).inboundSignal("locationIn",
   "String",
   function (v) {
   ...
   var providers = $Dict.fromList(_U.list([{ctor: "_Tuple2"
                                           ...
                                           $OnePageStack$Provider$Post.postProvider(A2($Path$Url._op["</>"],basePath,"posts")))}]));
   var tasks = Elm.Native.Task.make(_elm).performSignal("tasks",
   A3($OnePageStack$Server.server,
   A2($OnePageStack$Template.withTemplate,$Template.indexTemplate(basePath),$OnePageStack$Provider$Index.indexProvider(basePath)),
   providers,
   $OnePageStack$Server.currentLocation(locationIn)));
   return _elm.Main.values = {_op: _op,basePath: basePath,providers: providers,main: main};
};
